...
export default class AuthService {
  ...
  static async checkDuplicateLogin(data: JSObject) {
    ...
    if (user)
      throw new HttpErrors("Login already in use", HttpErrors.types.BadRequest);
  }
  ...
  static verifyToken(req: JSObject) {
    ...
    const refreshTime = Math.ceil(
      Math.abs(new Date().getTime() - new Date(+req.jwt.iat * 1000).getTime()) /
        1000
    );
    if (
      refreshTime >
      Number(process.env.JWT_TOKEN_SECRET_LIFETIME_IN_MILLISECONDS)
    ) {
  ...
  static async signToken(data: JSObject) {
    ...
    if (!passwordIsValid)
      throw new HttpErrors("Password is invalid", HttpErrors.types.BadRequest);
  ...
  static async checkUserType(req: JSObject, type: string) {
    ...
    const userType = await AuthService.UserTypesServiceModel.findById(
      user.userTypeId
    );
